...
function TestTakingPage() {
  ...
  const handleSubmitAllTestCases = async () => {
    ...
    try {
      const config = { headers: { Authorization: `Bearer ${token}` } };
      const res = await axios.post(
        `${process.env.REACT_APP_API_URL}/api/questions/${selectedQuestion.id}/submit`,
        {
          code,
          languageId: Number(selectedLanguage)
        },
  ...
  const handleAutoSubmitTest = async () => {
    ...
    try {
      const config = { headers: { Authorization: `Bearer ${token}` } };
      const res = await axios.post(
        `${process.env.REACT_APP_API_URL}/api/questions/${selectedQuestion.id}/submit-test`,
        {
          code,
          languageId: Number(selectedLanguage),
          testId: test?.id
  ...
  useEffect(() => {
    ...
    const wsUrl = process.env.REACT_APP_API_URL || 'ws://localhost:8080/ws';
    ws = new window.WebSocket(`${wsUrl}/test/${testId}/user/${user.id}`);
    ws.onopen = () => {
      // Optionally authenticate or register
    };
    ws.onmessage = (event) => {
      try {
        const data = JSON.parse(event.data);
        if (data.type === 'time-ended' && !isLocked && !autoSubmitRef.current) {
          autoSubmitRef.current = true;
          setIsLocked(true);
          handleAutoSubmitTest();
          setTimeout(() => {
