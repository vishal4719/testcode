...
function AdminDashboard() {
  ...
  useEffect(() => {
    async function fetchStatus() {
      try {
        setLoading(true);
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/auth/user-login-enabled`);
    ...
    async function fetchDomains() {
      try {
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/admin/allowed-domains`, {
          headers: {
  ...
  const handleChange = async (e) => {
    ...
    setError('');
    try {
      await axios.post(`${process.env.REACT_APP_API_URL}/api/auth/set-user-login-enabled`, { enabled }, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
  ...
  const handleAddDomain = async (e) => {
    ...
    try {
      await axios.post(
        `${process.env.REACT_APP_API_URL}/admin/add-domain?domain=${encodeURIComponent(newDomain.trim())}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`
  ...
  const handleRemoveDomain = async (domainToRemove) => {
    try {
      await axios.delete(`${process.env.REACT_APP_API_URL}/admin/remove-domain`, {
