...
//    }
    ...
    public ResponseEntity<String> logoutCurrentUser() {
        try {
            ...
            String email = authentication.getName(); // username from JWT
            userService.logoutUser(email);           // clear session token
    ...
    public ResponseEntity<?> login(@RequestBody Map<String, String> loginRequest) {
        ...
        try {
            ...
            boolean isAdmin = user.getRoles() != null &&
                    user.getRoles().stream().anyMatch(r -> r.equalsIgnoreCase("ADMIN"));
            ...
            // Reject if already logged in from another device
            if (user.getCurrentSessionToken() != null) {
                return ResponseEntity.status(403).body(Map.of("message", "User already logged in on another device."));
            }
            ...
            userService.loginUser(email, token); // Save token to DB
            userService.updateLastLogin(email);  // Update timestamp
